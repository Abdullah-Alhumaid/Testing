...
<body>
<?php
  // Connect to the database and fetch the company data
  $db = new mysqli('localhost', 'root', '', 'COOP');
  $result = $db->query('SELECT company_id, company_name FROM companies');

  // Loop through the results and display the company names and IDs
  if ($result->num_rows > 0) {
    echo 'Current companies: ';
    while ($row = $result->fetch_assoc()) {
      echo '(' . $row['company_id'] . '=' . $row['company_name'] . '), ';
    }
  } else {
